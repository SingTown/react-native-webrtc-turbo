...
using LocalCandidate =
    NativeDatachannelTurboModuleLocalCandidate<int, std::string, std::string>;
template <>
struct Bridging<LocalCandidate>
    : NativeDatachannelTurboModuleLocalCandidateBridging<LocalCandidate> {};

using Message = NativeDatachannelTurboModuleMessage<int, std::string>;
template <>
struct Bridging<Message>
    : NativeDatachannelTurboModuleMessageBridging<Message> {};

class DatachannelImpl
    : public NativeDatachannelTurboModuleCxxSpec<DatachannelImpl> {
  public:
	DatachannelImpl(std::shared_ptr<CallInvoker> jsInvoker);

	int createPeerConnection(jsi::Runtime &rt,
	                         std::vector<std::string> servers);
	void closePeerConnection(jsi::Runtime &rt, int pc);
	void deletePeerConnection(jsi::Runtime &rt, int pc);

	std::string createOffer(jsi::Runtime &rt, int pc);
	std::string createAnswer(jsi::Runtime &rt, int pc);

	void setLocalDescription(jsi::Runtime &rt, int pc, std::string type);
	std::string getLocalDescription(jsi::Runtime &rt, int pc);

	void setRemoteDescription(jsi::Runtime &rt, int pc, std::string sdp,
	                          std::string type);
	...
	void addRemoteCandidate(jsi::Runtime &rt, int pc, std::string candidate,
	                        std::string mid);

	void onLocalCandidate(int pc, std::string candidate, std::string mid);
	void onMessage(int pc, std::string data);
