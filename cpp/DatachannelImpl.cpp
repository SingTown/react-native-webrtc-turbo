...
{
    void localDescriptionCallback(int pc, const char *sdp, const char *type, void *user_ptr)
    {
        DatachannelImpl *impl = static_cast<DatachannelImpl *>(user_ptr);
        impl->onLocalDescription(pc, sdp, type);
    }

    void localCandidateCallback(int pc, const char *cand, const char *mid, void *user_ptr)
    {
        DatachannelImpl *impl = static_cast<DatachannelImpl *>(user_ptr);
        impl->onLocalCandidate(pc, cand, mid);
    }

    DatachannelImpl::DatachannelImpl(std::shared_ptr<CallInvoker> jsInvoker) : NativeDatachannelTurboModuleCxxSpec(std::move(jsInvoker)) {}

    int DatachannelImpl::createPeerConnection(jsi::Runtime &rt, std::vector<std::string> servers)
    {
    ...
    void DatachannelImpl::setLocalDescription(jsi::Runtime &rt, int pc, std::string type)
    {
    ...
    void DatachannelImpl::setRemoteDescription(jsi::Runtime &rt, int pc, std::string sdp, std::string type)
    {
    ...
    void DatachannelImpl::addRemoteCandidate(jsi::Runtime &rt, int pc, std::string candidate, std::string mid)
    {
        rtcAddRemoteCandidate(pc, candidate.c_str(), mid.c_str());
    }

    void DatachannelImpl::onLocalDescription(int pc, std::string sdp, std::string type)
    {
        LocalDescription desc{pc, sdp, type};
        emitOnLocalDescription(desc);
    }

    void DatachannelImpl::onLocalCandidate(int pc, std::string candidate, std::string mid)
    {
