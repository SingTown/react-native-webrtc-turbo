...
- (BOOL)setupCameraWithError:(NSError **)error {
	...
	if (!camera) {
		if (error) {
			*error = [NSError
			    errorWithDomain:@"CameraError"
			               code:1001
			           userInfo:@{
				           NSLocalizedDescriptionKey : @"No camera device found"
			           }];
		}
		return NO;
	}
	...
	if ([_session canAddInput:input]) {
		...
		if (error) {
			*error = [NSError errorWithDomain:@"CameraError"
			                             code:1002
			                         userInfo:@{
...
- (void)captureOutput:(AVCaptureOutput *)output
	...
	for (size_t i = 0; i < height / 2; ++i) {
		memcpy(frame->data[1] + i * frame->linesize[1], srcUV + i * srcUVStride,
		       width);
	}
